    ...
    * the extra L2 penalty term will only ever add to the loss
    ...
    def fit(self, x_NF, y_N):
        ...
        * trace_L1_norm_of_grad contains L1 norm of grad after every step
        ...
        print("Shape of Transformed Data", xbias_NG.shape)
        ...
        for iter_id in range(self.num_iterations + 1):
            ...
            if self.verbose:
                if iter_id < 20 or (iter_id % 100 == 0) or (iter_id % 100 == 1):
...
#        log_loss = -y_N.dot(np.log(p))-(1-y_N).dot(np.log(1-p))
        ...
        log_loss2 = y_N.dot(np.logaddexp(zeroN, -z)) + (1-y_N).dot(np.logaddexp(zeroN, z))
        
        #Another Version: Written by my self
        #log_loss2  = -y_N.dot(self.logsigmoid(z)) - (1-y_N).dot(self.logsigmoid(-z))
        
        #print("check log loss",  np.logaddexp(zeroN, -z), self.logsigmoid(z))
        ## Add two terms together and return sum divided by N * np.log(2)
        return (L2_loss + log_loss2) / denom
    
    def logsigmoid(self,z):
        N=int(z.size)
        res=np.zeros(N)
#        for i in range(N):
#            if (z[i]>=0):
#                res[i]=np.log(1/(1+np.exp(-z[i])))
#            else:
#                res[i]=np.log(1/(1+np.exp(z[i])))+z[i]
        zAbs=np.abs(z)
        res=np.log(1/(1+np.exp(-zAbs)))+np.multiply((z<0),z)
        return res
